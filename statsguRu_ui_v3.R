...
ui <- fluidPage(
  tags$head(tags$style(
    HTML(".sidebar-menu li a { font-size: 30px;padding: 10px }")
  )),
  ...
  dashboardPage(
    ...
    dashboardSidebar(
      ...
      column(
        width = 12,
        actionButton("do", strong("Click to Update !")),
    ...
    dashboardBody(tabItems(
      tabItem(tabName = "dashboard",
              navbarPage(
                "Tabs",
                tabPanel("How to Use !", h1("Coming Soon! Keep Checking the tab")),
                tabPanel(
                  "Batting Statistics",
                  h3(
      ...
      tabItem(
        tabName = "about",
        column(width = 12, h2(strong(
          "About This Shiny App :"
        ))),
        column(
          width = 12,
          h2("A Dashboard to see visualizations of a Cricketer's profile")
        ),
        column(width = 12, h2(strong("Package Used :"))),
        column(width = 12, h2("statsguRu")),
        column(width = 12, h2(
          "https://github.com/npranav10/statsguRu"
        )),
